// 입력값 검증(유효성 체크)
let id = document.getElementById("user-id");
let pw1 = document.getElementById("user-pw1");
let pw2 = document.getElementById("user-pw2");

// 표현식에 공백이 있으면 인식이 안됨
let pw_pat1 = /[0-9]/;
let pw_pat2 = /[a-z]/;
let pw_pat3 = /[A-Z]/; 
let pw_pat4 = /[ ~!@#$%^&*?]/;

// onchange 속성 사용
id.onchange = checkId; // checkId 함수 호출 괄호는 생략한다

// 이벤트 처리기 함수를 사용했을 경우
// id.addEventListener("change", checkId);

pw1.onchange = checkPw;
pw2.onchange = comparePw;

function checkId(){
    if(id.value.length < 4 || id.value.length > 15){
        alert("4~15자리의 영문과 숫자로 입력해 주세요");
        // id.value = "";
        id.select(); // 선택 범위 지정
    }
}
function checkPw(){
    if(pw1.value.length < 8){
        alert("비밀번호는 숫자, 영어 대,소문자, 특수문자 포함 8자 이상입니다.");
        pw1.value = ""; // 기존 문자 지우기
        pw1.focus();
        return 0;
    }
    if(!pw_pat1.test(pw1.value)){
        alert("숫자를 포함시켜주세요");
        pw1.value = ""; // 기존 문자 지우기
        pw1.focus();
        return false;
    }
    if(!pw_pat2.test(pw1.value)){
        alert("소문자를 포함시켜주세요");
        pw1.value = ""; // 기존 문자 지우기
        pw1.focus();
        return false;
    }
    if(!pw_pat3.test(pw1.value)){
        alert("대문자를 포함시켜주세요");
        pw1.value = ""; // 기존 문자 지우기
        pw1.focus();
        return false;
    }
    if(!pw_pat4.test(pw1.value)){
        alert("특수문자는 ~!@#$%^&*? 그리고 스페이스(공백) 입니다.");
        pw1.value = ""; // 기존 문자 지우기
        pw1.focus();
        return false;
    }
}
function comparePw(){
    if(pw1.value != pw2.value){
        alert("비밀번호가 일치하지 않습니다.");
        pw2.value = ""; // 기존 문자 지우기
        pw2.focus();
    }
}